 ...
 * ? word - to perform a search on all DB
 * /id_ #ID - to get the phone of the element identified by the #ID
 * /pos_ #ID - to get the position of the element identified by the #ID
 * 
 */
class mainloop {
    ...
    function getCollectionUsers() {
        $DB = MONGODB;
        return (new MongoDB\Client)->$DB->users;
    }
    ...
    function getCollection($name) {
        ...
        switch ($name) {
            case 'users':
                ...
                break;
            case 'access':
                $collection = (new MongoDB\Client)->$DB->access;
                break;
            default:
                $collection = NULL;
                break;
    ...
    function start($telegram, $update) {
       ...
       // $this->collection = $this->getCollectionUsers();
        ...
        $access = $this->getCollection('access');
        $access->insertOne([
...
// the following word after ? is the key for the search
        elseif (preg_match('/^\?/', $this->text) || "kit" == $this->text) {
            $this->sendListResult($telegram);
        }
...
//return $content;
    ...
    function create_keyboard_temp($telegram) {
        $option = array(["KEYWORDS", "kit"], ["Help", "Posizione/Position"]);
